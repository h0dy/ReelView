...
const (
	TokenTypeAccess TokenType = "ReelView-access"
)

func MakeJWT(userID uuid.UUID, tokenSecret string, expiresIn time.Duration) (string, error) {
	claims := jwt.RegisteredClaims{
		Issuer:    string(TokenTypeAccess), // Issuer: who created the JWT
		IssuedAt:  jwt.NewNumericDate(time.Now().UTC()),
		ExpiresAt: jwt.NewNumericDate(time.Now().UTC().Add(expiresIn)),
		Subject:   userID.String(), // Subject: whose token is this (user id)
	}
	...
	return jwtToken.SignedString([]byte(tokenSecret)) // return complete JWT string
}
